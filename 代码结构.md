...
# evaluate
## usage
1. 服务器运行 `CUDA_VISIBLE_DEVICES=0 python simplify_work/server/server_code/get_data_from_client.py` cuda_visible可以不加
...
```
CUDA_VISIBLE_DEVICES=0 python -m lerobot.scripts.server.policy_server \
    --host="10.10.16.18" \
    --port=9000
```
...
rsync -avz --exclude='*.safetensors' user@server:/path/to/outputs/train/ outputs/train/
...
## 训练
1. configs/trian.py里面加参数 add_location_to_state
2. 输入数据处理 在script/train.py里面，FilteredBatchLoader，得到state（加4个值，一共10，不会超过32，smolvla里面能正常prepare）
3. smolvla
    1. 重置state_projection的最后四列
